...
fileName_ref_in = strcat(path,'\C_con_kcatEC_rxn\',fileName_ref);

%% Set the parameter values
write = 0;
write_all = 1; % Write all
%% Calculate the protein abundance from gene
model_out = fill_the_EC_gap(fileName_ref_in,fileName_kcat_in,write,path,write_all);
% save
save(strcat(output_folder,'\','model_out.mat'),'model_out');

function out = fill_the_EC_gap(fileName_ref_in,fileName_kcat_in,write,path,write_all)
...
disp([newline '1.1.1.x, # of kcat updte: ' num2str(numb_of_kcat_without_0) ' out of ' num2str(length(df_ref.kcat)),...
        ' (' num2str((numb_of_kcat_without_0/length(df_ref.kcat)*100)) '%) from Drosophila, ' num2str(sum(df_ref.kcat==0)),...
        ' not filled'])
disp(['1.1.1.x, # of kcat updte: ' num2str(numb_of_kcat_without_0) ' out of ' num2str(non_empty_indices),...
        ' (' num2str((numb_of_kcat_without_0/non_empty_indices*100)) '%) from Drosophila, ' num2str(non_empty_indices-numb_of_kcat_without_0),...
        ' not filled'])
...
df_ref.update_D = 'EC titration was used';
out = df_ref;
