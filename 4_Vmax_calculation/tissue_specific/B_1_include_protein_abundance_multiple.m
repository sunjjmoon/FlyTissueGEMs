...
%% multiple models
model_in_tmp = model_in;

for p = 1:length(model_in_tmp) 
model_in = model_in_tmp{1,p};
...
a = df_all{:,1}; % gene name
a_val = df_all{:,2:end};
% because model is not in the order of the excel sheet, find the column
% that maches to the model ID
id_tmp = model_in.id;
idx_tmp = find((ismember(string(df_all_var(2:end)'),string(id_tmp))));
a_val = a_val(:,idx_tmp);
...
[Lia,~] = ismember(a(:,1),b(i,1)); % Find whethter a (from calculated val) is part of element in b (model gene). 
%     count = count +sum(Lia);
%     disp(count)
...
disp(model_in.id)
disp(['# of filled_protein: ' num2str(numb_of_filled_protein) ' out of ' num2str(total_elements) ' ('...
        num2str(percentage_of_rxn_with_EC) ' %)' newline ...
        '# of empty protein values: ' num2str(sum(empty_indices)) newline]);
...
%         protein_ppm = 10.^(slope.*log10(a_val(idx,1))+y_int);
