%% Goal:
% - To simulate the FVA analysis for HSD-muscle-GEM
% Input:
...
%% Load all models
model_out_cbra = load(fullfile('models', 'model_out_cbra_u.mat'));
...
for i = 1:length(model_out_cbra)
    ...
    [out, subSysModel] = run_fva(model, model_id, reductionFactor, scaling_factor);
    % Store results
...
function [FVAsolution, subSysModel] = run_fva(model, model_id, reductionFactor, scaling_factor)
...
%% 5. Apply constraints in central carbon metabolic reactions, including GAPDH
% GAPDH
for i = 1:length(model)
    list = {'MAR04373'};  
    for j = 1:length(list)
...
% OGDH
for i = 1:length(model)
    list = {'MAR04209','MAR05297','MAR06411','MAR06413'};  %
    for j = 1:length(list)
        list_in = list(j);
        idx_in = find(ismember(model.rxns,list_in));
        if strcmp(model_id,'HSD') == 1 % HSD
