...
%% Load the input
load(strcat(pathway,'\3_linkEC2rxn\','out_empty2.mat'));
load(strcat(pathway,'\3_linkEC2rxn\','fruitfly2_ECupd.mat'));

%% Load the Gene-EC inro
fileIn=strcat(pathway,'\files\','EC_numbers');

%% Run the functions
[out_empty3,gem_uu,T2] = run_genelistOfEmptyEC(fileIn,out_empty2,gem_u,save_dir);
movefile('out_empty3.mat',subfolder);
movefile('fruitfly3.mat',subfolder);
writetable(T2,strcat(subfolder,'\updatedECINfo.xlsx'))

function [out_empty3,gem_uu,T2] = run_genelistOfEmptyEC(fileIn,out_empty2,gem_u,save_dir)
% Goal:
...
T = table(out_empty3);
filename = fullfile(save_dir,'genes_assoc_EC_basedOn_genes.xlsx');
write(T,filename);
save('out_empty3.mat','out_empty3')
save('fruitfly3.mat','fruitfly3')
end