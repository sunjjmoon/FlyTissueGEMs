% Goal:
%   - To derive the enzyme abundance from the gene expression of FCA of 32
%   different tissues. 
...
fileName_mrna = 'FCA_pseudobulk_table_32_clusters_u';
...
save(strcat(output_folder,'\','gene2protein.mat'), 'gene2protein');
...
%2023. 10. 23. 
correction_factor = 10; % Since the values are not rpkm or fpkm, it is not technically the rpkm values. but using this
% it makes at least in the same order of magnitude.
...
protein_ppm = protein_copyNumb.*df_u(:,end-2).*avg_aaMass./avo./vol_s2*10^3; % multiply by length of aa and mass, which will give Da/protein molecules
...
% protein_mmol_gDCW = protein_ppm./1000./maa./fdw;
